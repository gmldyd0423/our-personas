...
# ///
"""
Personas CLI - Setup tool for Personas Projects

Usage:
    uvx personas-cli.py init <project-name>
    uvx personas-cli.py init .
    uvx personas-cli.py init --here

Or install globally:
    uv tool install --from personas-cli.py personas-cli
    personas init <project-name>
    personas init .
    personas init --here
"""
...
app = typer.Typer(
    name="personas",
    help="Setup tool for Personas spec-driven development projects",
...
def callback(ctx: typer.Context):
    ...
    if ctx.invoked_subcommand is None and "--help" not in sys.argv and "-h" not in sys.argv:
        show_banner()
        console.print(Align.center("[dim]Run 'personas --help' for usage information[/dim]"))
        console.print()
...
def check_tool(tool: str, tracker: StepTracker = None) -> bool:
    ...
    # See: https://github.com/dauquangthanh/our-personas/issues/123
...
def init_git_repo(project_path: Path, quiet: bool = False) -> Tuple[bool, Optional[str]]:
    ...
    try:
        ...
        subprocess.run(["git", "commit", "-m", "Initial commit from Personas template"], check=True, capture_output=True, text=True)
        if not quiet:
...
def download_template_from_github(ai_assistant: str, download_dir: Path, *, script_type: str = "sh", verbose: bool = True, show_progress: bool = True, client: httpx.Client = None, debug: bool = False, github_token: str = None) -> Tuple[Path, dict]:
    ...
    pattern = f"our-personas-template-{ai_assistant}-{script_type}"
    matching_assets = [
...
def ensure_executable_scripts(project_path: Path, tracker: StepTracker | None = None) -> None:
    """Ensure POSIX .sh scripts under .personas/scripts (recursively) have execute bits (no-op on Windows)."""
    if os.name == "nt":
        return  # Windows: skip silently
    scripts_root = project_path / ".personas" / "scripts"
...
def init(
    ...
    Examples:
        ...
        personas init my-project --ai copilot --no-git
        personas init --ignore-agent-tools my-project
        personas init . --ai claude         # Initialize in current directory
        personas init .                     # Initialize in current directory (interactive AI selection)
        personas init --here --ai claude    # Alternative syntax for current directory
...
def check():
    ...
    console.print("\n[bold green]Personas CLI is ready to use![/bold green]")
